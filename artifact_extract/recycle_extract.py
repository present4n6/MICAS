...
def extract_artifact(argv):
	...
	cur = db.cursor()
	cur.execute("select inum from mftinfo where `FN name`='$Recycle.Bin'")
	inum = cur.fetchall()[0][0]
	cur.execute("select * from mftinfo where `FN pdfme`="+str(inum))
	recycle_list= cur.fetchall()
	for row in recycle_list:
		os.mkdir("%s/%s/%s/%s/%s" %(argv[1], argv[2], image_dir,"recycle", row[F_NAME]))
		cur.execute("select inum, `FN name` from mftinfo where `FN pdfme`="+str(row[0]))
