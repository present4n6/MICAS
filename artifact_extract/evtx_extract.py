...
def extract_artifact(argv):
	...
	cur = db.cursor()
	cur.execute("SELECT * FROM "+image_dir+"_ mftinfo WHERE `FN name`  LIKE '%.evtx'")

	for row in cur.fetchall():
		print(row)
		print(str(row[INUM]) + " " + row[F_NAME])
		command = "extractdata -i" + " " + argv[1]+"/" + casename+ "/"+  disk_image + " " + "-o 0 -q" + " " + str(row[INUM]) + " " + "-e" + " " + argv[1] + "/" + casename+ "/" + image_dir + "/eventlog/" + row[F_NAME].replace(" ", "")
